    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 lg:gap-6">
            {[
                ...
                { label: t('dashboard.devicesConnected'), value: `${connectedDevices}/${totalDevices}`, icon: TrendingUp, color: 'text-teal-600', bg: 'bg-teal-50' }
            ].map((card, idx) => (
                <div key={idx} className="bg-white rounded-2xl shadow-sm border border-gray-200 p-4 sm:p-5 lg:p-6 min-w-0 flex flex-col justify-between group hover:shadow-md transition-all">
                    <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                            <p className="text-[10px] lg:text-[11px] font-black text-gray-400 uppercase tracking-widest mb-1 truncate">{card.label}</p>
